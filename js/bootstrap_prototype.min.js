...
$$('[data-toggle="collapse"]').each(function(a){var b=a.readAttribute("href"),b=b.replace(/.*(?=#[^\s]+$)/,"").replace("#",""),b=a.readAttribute("data-target")||b,c={toggle:!1};a.hasAttribute("data-parent")&&(c.parent=a.readAttribute("data-parent").replace("#",""));b=$(b);b.hasClassName("in")?a.addClassName("collapsed"):a.removeClassName("collapsed");b.store("bootstrap:collapse",new BootStrap.Collapse(b,c))});document.on("click",'[data-toggle="collapse"]',function(a){var b=a.findElement().readAttribute("href"),
b=b.replace(/.*(?=#[^\s]+$)/,"").replace("#","");a=a.findElement().readAttribute("data-target")||a.preventDefault()||b;$(a).retrieve("bootstrap:collapse").toggle()})});